...
class UserSessionAnalytics(Base):
    ...
    def end_session(self):
        ...
        if self.started_at:
            ...
            self.bounce_session = (
                self.duration_seconds < 30 and 
                self.page_views <= 1 and 
                self.clicks <= 1
            )
...
class ABTestParticipant(Base):
    ...
    # Assignment
    variant = Column(String(50), nullable=False)  # control, treatment_a, treatment_b, etc.
...
class DetectionAccuracyLog(Base):
    ...
    detection_method = Column(String(30), nullable=False)  # google_vision, tesseract, manual
    ...
    def set_ground_truth(self, correct_label: str):
        ...
        self.is_correct = (self.final_result == correct_label) if self.final_result else False
...
class RevenueEvent(Base):
    ...
    # Event details
    event_type = Column(String(50), nullable=False, index=True)  # subscription, upgrade, usage_fee
