...
async def get_user_dashboard(
    ...
    processing_jobs = (
        ...
        .filter(
            ...
            == "completed",  # Only include completed jobs for accurate metrics
        )
        .all()
    )
    ...
    corrections_to_detected_photos = (
        db.query(UsageLog)
        .filter(
            UsageLog.user_id == current_user.id,
            UsageLog.action_type == ActionType.MANUAL_LABEL,
            UsageLog.created_at >= datetime.utcnow() - timedelta(days=days),
            func.json_extract(UsageLog.details, "$.bib_number")
            != "unknown",  # Not "no bib visible"
            func.json_extract(UsageLog.details, "$.was_unknown")
            is False,  # Was originally detected
        )
        .count()
    )
    ...
    photos_with_bibs = max(
        1, total_photos_processed - no_bib_labels
    ...
    if total_photos_processed > 0 and total_processing_time > 0:
        avg_processing_time = (
            total_processing_time / total_photos_processed
        ) * 1000  # Convert to ms
    ...
    return {
        ...
        "detection_stats": {
            "google_vision_detections": sum(
                job.google_vision_detections or 0 for job in processing_jobs
            ),
            "tesseract_detections": sum(
                job.tesseract_detections or 0 for job in processing_jobs
            ),
            "manual_labels": total_manual_labels,
            "no_bib_labels": no_bib_labels,
            "manual_corrections": manual_corrections,
...
async def get_admin_dashboard(
    ...
    # Get recent alerts
    recent_alerts = (
        db.query(AlertHistory).order_by(desc(AlertHistory.triggered_at)).limit(10).all()
    )
...
async def get_user_analytics(
    ...
    if sort_by == "photos":
        users = (
            db.query(User).order_by(desc(User.total_photos_uploaded)).limit(limit).all()
        )
    ...
    return {"users": user_analytics, "total_count": len(users), "sort_by": sort_by}
...
async def get_detection_performance(
    ...
    accuracy_stats = (
        db.query(
            ...
            func.avg(DetectionAccuracyLog.tesseract_confidence).label(
                "avg_tes_confidence"
            ),
        )
    ...
    return {
        "overview": {
            ...
            "avg_google_vision_confidence": round(
                accuracy_stats.avg_gv_confidence or 0, 2
            ),
            "avg_tesseract_confidence": round(
                accuracy_stats.avg_tes_confidence or 0, 2
            ),
        },
        "method_breakdown": [
            {
...
async def get_user_cohorts(
    ...
    cohorts = (
        ...
        .order_by(desc(UserRetentionCohort.cohort_month))
        .all()
    )
...
async def get_system_metrics(
    metric_type: Optional[SystemMetricType] = Query(
    ...
    for metric in metrics:
        ...
        if not aggregated[type_key][
            "latest"
        ] or metric.timestamp > datetime.fromisoformat(
            aggregated[type_key]["latest"]["timestamp"].replace("Z", "+00:00")
        ):
    ...
    return {"period_hours": hours, "metrics": aggregated}
...
async def export_system_metrics(
    ...
    try:
        ...
        media_type = "text/csv" if format.lower() == "csv" else "application/json"
        filename = (
            f"system_metrics_{datetime.utcnow().strftime('%Y%m%d')}.{format.lower()}"
        )
...
async def track_conversion_event(
    ...
    existing = (
        db.query(ConversionFunnel)
        .filter(
            ConversionFunnel.user_id == current_user.id, ConversionFunnel.step == step
        )
        .first()
    )
...
def get_top_users_by_activity(
    db: Session, days: int, limit: int = 10
) -> List[Dict[str, Any]]:
    ...
    top_uploaders = (
        ...
        .join(UsageLog)
        .filter(
            UsageLog.action_type == ActionType.UPLOAD, UsageLog.created_at >= since_date
        )
        .group_by(User.id)
        .order_by(desc("total_photos"))
        .limit(limit)
        .all()
    )
...
async def get_system_health(
    ...
    try:
        ...
        upload_dir_size = sum(
            f.stat().st_size for f in Path("uploads").rglob("*") if f.is_file()
        ) / (
            1024**2
        )  # MB
        export_dir_size = sum(
            f.stat().st_size for f in Path("exports").rglob("*") if f.is_file()
        ) / (
            1024**2
        )  # MB
        ...
        # Database metrics
        db_file_size = (
            Path("tag_photos.db").stat().st_size / (1024**2)
            if Path("tag_photos.db").exists()
            else 0
        )
        ...
        active_sessions = (
            db.query(UserSession).filter(UserSession.is_active is True).count()
        )
        active_jobs = (
        ...
        queue_size = (
            ...
            .filter(ProcessingJobDB.status == "pending")
            .scalar()
            or 0
        )
        ...
        await analytics_service.record_system_metric(
            db,
            SystemMetricType.PROCESSING_QUEUE_SIZE,
            "queue_size",
            queue_size,
            "count",
        )
        await analytics_service.record_system_metric(
            db,
            SystemMetricType.ACTIVE_SESSIONS,
            "active_sessions",
            active_sessions,
            "count",
        )
...
async def get_active_alerts(
    ...
    return {
        ...
        "critical_count": len(
            [a for a in alert_data if a["alert_level"] == "critical"]
        ),
...
async def create_alert_rule(
    rule: dict, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)
):
...
async def get_performance_trends(
    ...
    metrics_data = (
        ...
        .filter(SystemMetric.timestamp >= since_date)
        .group_by(
            SystemMetric.metric_type,
            SystemMetric.metric_name,
