...
async def batch_update_labels(
    ...
    photos = (
        db.query(PhotoDB)
        .filter(
            PhotoDB.photo_id.in_(valid_photo_ids), PhotoDB.user_id == current_user.id
        )
        .all()
    )
    ...
    logger.info(
        f"Batch update labels: {success_count} photos updated for user {current_user.id}"
    )
...
async def batch_delete_photos(
    ...
    batch_op = BatchOperationDB(
        ...
        operation_data={"photo_ids": [p.photo_id for p in photos]},
        affected_count=len(photos),
    ...
    logger.info(
        f"Batch delete: {success_count} photos deleted for user {current_user.id}"
    )
...
async def batch_reprocess_photos(
    ...
    if not request.force:
        processing_photos = [
            p for p in photos if p.processing_status == ProcessingStatus.PROCESSING
        ]
    ...
    if success_count > 0:
        ...
        reprocess_job = job_service.create_job(
            ...
            [
                p.photo_id
                for p in photos
                if p.processing_status == ProcessingStatus.PENDING
            ],
            debug=False,
        )
    ...
    logger.info(
        f"Batch reprocess: {success_count} photos queued for user {current_user.id}"
    )
...
async def undo_batch_operation(
    ...
    if not batch_op:
        raise HTTPException(
            status_code=404, detail="Operation not found or cannot be undone"
        )
