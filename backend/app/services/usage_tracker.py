...
class UsageTracker:
    ...
    def get_user_stats(self, db: Session, user_id: int, days: int = 30) -> dict:
        ...
        stats = {
            ...
            "uploads": len(
                [log for log in usage_logs if log.action_type == ActionType.UPLOAD]
            ),
            "processes": len(
                [log for log in usage_logs if log.action_type == ActionType.PROCESS]
            ),
            "exports": len(
                [log for log in usage_logs if log.action_type == ActionType.EXPORT]
            ),
            "logins": len(
                [log for log in usage_logs if log.action_type == ActionType.LOGIN]
            ),
            # Photo statistics
            "total_photos_uploaded": sum(
                log.photo_count
                for log in usage_logs
                if log.action_type == ActionType.UPLOAD
            ),
            "total_photos_processed": sum(
                log.photo_count
                for log in usage_logs
                if log.action_type == ActionType.PROCESS
            ),
    ...
    def get_system_stats(self, db: Session, days: int = 30) -> dict:
        ...
        # Activity statistics
        total_actions = (
            db.query(UsageLog).filter(UsageLog.created_at >= since_date).count()
        )
        unique_active_users = (
        ...
        total_jobs = (
            db.query(ProcessingJob)
            .filter(ProcessingJob.created_at >= since_date)
            .count()
        )
        completed_jobs = (
            db.query(ProcessingJob)
        ...
        successful_actions = (
            db.query(UsageLog)
            .filter(UsageLog.created_at >= since_date, UsageLog.success.is_(True))
            .count()
        )
        ...
        return {
            ...
            # Performance metrics
            "success_rate_period": (
                (successful_actions / total_actions * 100) if total_actions > 0 else 100
            ),
            "job_completion_rate": (
            ...
            # Averages
            "avg_photos_per_user": (
                photos_uploaded / unique_active_users if unique_active_users > 0 else 0
            ),
            "avg_jobs_per_user": (
                total_jobs / unique_active_users if unique_active_users > 0 else 0
            ),
        }
